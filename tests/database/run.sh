...
task_field_repo=${FIELD_REPO_PATH:-../Invenio Field}
[ -d "$task_field_repo/supabase/migrations" ] || { echo "Set FIELD_REPO_PATH to the Field migration baseline" >&2; exit 1; }
apply_sql tests/database/bootstrap.sql
for task_migration in "$task_field_repo"/supabase/migrations/00[1-8]_*.sql; do apply_sql "$task_migration"; done
...
apply_sql "$task_field_repo"/supabase/migrations/011_msr_table_definitions.sql
apply_sql "$task_field_repo"/supabase/migrations/20260306230739_009_secure_users_rls.sql
apply_sql "$task_field_repo"/supabase/migrations/010_fix_rls_and_add_msr_project_id.sql
